...
class KDAAttnBackend(MambaAttnBackendBase):
    ...
    def __init__(self, model_runner: ModelRunner):
        ...
        # mamba_cache.conv is [..., kernel-1, dim] while conv_states_shape expects
        # the window length (kernel-1) at shape[-1], hence the transpose. This is
        # the KDA (K-1, channel) layout, transposed vs GDN's (channel, K-1).
    ...
    def forward_extend(
        ...
        **kwargs,
        ...
        # Snapshot the last conv window of the aligned prefix into the persistent
        # track slots for prefix caching. KDA conv[0] is already (K-1, channel), so
        # the pre-transpose mixed_qkv[track_conv_indices] rows match directly (no
        # transpose, unlike GDN whose conv is (channel, K-1)).
        if self.forward_metadata.has_mamba_track_mask:
