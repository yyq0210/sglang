...
def _corrupted(ref: torch.Tensor, mode: str) -> torch.Tensor:
    ...
    noise = torch.randn(ref.shape, generator=gen, device=ref.device) * (std * _NOISE_SCALE)
...
def _prefix_kv_locs(req_to_token: torch.Tensor, forward_batch: "ForwardBatch") -> torch.Tensor:
    ...
    if forward_batch.forward_mode.is_decode():
        ...
        lens = pl if pl is not None else (forward_batch.seq_lens - forward_batch.extend_seq_lens)
