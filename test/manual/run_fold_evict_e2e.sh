#!/usr/bin/env bash
# Phase C2-b live fold-to-state KV eviction A/B on Qwen3-Next-80B.
#
#   Run: bash test/manual/run_fold_evict_e2e.sh <ARM>
#     ARM = baseline | fold | report | perf_baseline | perf_fold
#
...
ARM="${1:?usage: $0 <baseline|fold|report|perf_baseline|perf_fold>}"
...
case "$ARM" in
  baseline|fold)
    ...
    echo "[done] arm=$ARM -> $NEEDLE_OUT  (run baseline + fold, then: $0 report)"
    ;;
  perf_baseline|perf_fold)
